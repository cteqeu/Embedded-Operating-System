...
int main( void )
{
	...
	xQueue = xQueueCreate( 	1,						/* There is only one space in the queue. */
							sizeof(&value) );	/* Each space in the queue is large enough to hold a uint32_t. */
...
int readval=0;
	for( ;; )
	{
		/* Delay for 1 second. */
		vTaskDelay( x1second );
		readval = XGpio_DiscreteRead(&Gpio, BUTTON_CHANNEL);
...
static void prvRxTask( void *pvParameters )
{
	...
	for( ;; )
	{
		...
		xQueueReceive( 	xQueue,				/* The queue being read. */
						&Recdvalue,	/* Data is read into this address. */
						portMAX_DELAY );	/* Wait without a timeout for data. */
		XGpio_DiscreteWrite(&Gpio, LED_CHANNEL, Recdvalue);
